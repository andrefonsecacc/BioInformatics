public class NeedlemanWunsch {
	...
	/*
	 * Needleman-Wunsch Dynamic Programming Algorithm see
	 * http://amrita.vlab.co.in/?sub=3&brch=274&sim=1431&cnt=1 Runtime
	 * complexity: O(NxM), Space complexity: O(NxM) where N,M are lengths of the
	 * sequences
	 */
	public static AlignmentResult computeNWAlignment(String seq1, String seq2, SimpleAlignmentParameters parameters) {
		...
		for (int i = 1; i < seq1.length() + 1; i++) {
			for (int j = 1; j < seq2.length() + 1; j++) {
				...
				scoreMatrix[i][j] = Math.max(Math.max(matchCost, seq1GapCost), seq2GapCost);
			}
		}
